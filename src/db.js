...
export const db = {
    ...
    getSettings: async () => {
        ...
        const defaults = {
            material_m2: 4000,
            wall_light: 4500, wall_medium: 5500, wall_heavy: 7500,
            work_strobe: 1500, work_cable: 400,
            work_box: 1000, work_junction: 2500,
            work_point: 1500, work_lamp: 3000,
            work_automaton: 2500, work_shield_install: 5000,
            work_input: 15000, work_check: 20000,
            business_percent: 20,
            staff_percent: 80
        };
    ...
    upsertUser: async (telegramId, firstName, username, phone = null) => {
        ...
        try {
            ...
            `;
            const res = await client.query(query, [telegramId, firstName, username, phone]);
            return res.rows[0];
        } catch (e) {
    ...
    updateUserPhone: async (telegramId, phone) => {
        try {
            await pool.query('UPDATE users SET phone = $1, updated_at = NOW() WHERE telegram_id = $2', [phone, telegramId]);
            return true;
        } catch (e) {
    ...
    createOrder: async (telegramId, leadId) => {
        ...
        try {
            ...
            const userRes = await client.query('SELECT telegram_id, first_name, username, phone FROM users WHERE telegram_id = $1', [telegramId]);
    ...
    transferMoney: async (data) => {
        ...
        try {
            ...
            await client.query('UPDATE accounts SET balance = balance - $1, updated_at = NOW() WHERE id = $2', [amount, fromAccountId]);
            
            // 2. Ақшаны кіргізу (Plus)
            await client.query('UPDATE accounts SET balance = balance + $1, updated_at = NOW() WHERE id = $2', [amount, toAccountId]);
            ...
            await client.query(
                ...
                [userId, fromAccountId, amount, `Шығыс аударым: #${toAccountId}. ${comment}`]
            );

            await client.query(
                `INSERT INTO transactions (user_id, account_id, amount, type, category, comment, created_at)
                 VALUES ($1, $2, $3, 'income', 'transfer', $4, NOW())`,
                [userId, toAccountId, amount, `Кіріс аударым: #${fromAccountId}. ${comment}`]
            );
    ...
    addTransaction: async (data) => {
        const { userId, accountId, amount, type, category, comment, orderId } = data;
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
    ...
    try {
        ...
        await client.query(`
            ...
            CREATE TABLE IF NOT EXISTS settings (
                key TEXT PRIMARY KEY, value NUMERIC NOT NULL,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            );

            CREATE TABLE IF NOT EXISTS leads (
                id SERIAL PRIMARY KEY, user_id BIGINT REFERENCES users(telegram_id),
                area NUMERIC, wall_type TEXT, 
                total_work_cost NUMERIC, total_mat_cost NUMERIC,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            );
            ...
            CREATE TABLE IF NOT EXISTS orders (
                ...
                status TEXT DEFAULT 'new', assignee_id BIGINT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            );
            ...
            CREATE TABLE IF NOT EXISTS transactions (
                ...
                account_id INTEGER REFERENCES accounts(id), -- Қай шотта болды
        ...
        // Базалық шоттарды қосу (тек бос болса)
        ...
        const initialSettings = [
            ['material_m2', 4000], ['wall_light', 4500], ['wall_medium', 5500], ['wall_heavy', 7500],
            ['work_strobe', 1500], ['work_cable', 400], ['work_box', 1000], ['work_junction', 2500],
            ['work_point', 1500], ['work_lamp', 3000], ['work_automaton', 2500], ['work_shield_install', 5000],
            ['work_input', 15000], ['work_check', 20000],
            ['business_percent', 20], ['staff_percent', 80]
        ];
        ...
        console.log(`✅ [DB] Database initialized (Accounts & ERP v3.0)`);
    } catch (err) {
        console.error('💥 [DB FATAL] Init failed:', err);
        process.exit(1);
    } finally {
        client.release();
    }
