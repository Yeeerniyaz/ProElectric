 ...
 * @description Обработчик клиентской части (Client Controller v10.9.4 Enterprise).
 ...
 *
 * @module UserHandler
 * @version 10.9.4 (Senior Architect Bugfix Edition)
 */
...
export const UserHandler = {
  ...
  async saveOrderAction(ctx) {
    try {
      ...
      // 🔥 ИСПРАВЛЕНИЕ: Рассылка СМС (уведомлений) всем Бригадирам на Биржу
      try {
          ...
          await ctx.telegram
            .sendMessage(
              ...
              `⚡️ <b>НОВЫЙ ОБЪЕКТ НА БИРЖЕ!</b>\n` +
                `➖➖➖➖➖➖➖➖➖➖\n` +
                `💰 <b>Смета:</b> ${fmt(estimate.total.work)} ₸\n` +
  ...
  // 🔥 ИСПРАВЛЕНИЕ: Преобразование типов String() для проверки BIGINT
  async cancelOrderByUser(ctx, orderId) {
    try {
      const order = await OrderService.getOrderById(orderId);
      // BIGINT конвертируется в String в JS, поэтому нужно строгое приведение типов
