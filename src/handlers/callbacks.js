...
const formatKZT = (num) => new Intl.NumberFormat('ru-KZ', { style: 'currency', currency: 'KZT', maximumFractionDigits: 0 }).format(num);
    ...
    bot.on('callback_query', async (query) => {
        ...
        try {
            ...
            if (data.startsWith('status_')) {
                ...
                if (!cfg || !orderId) return bot.answerCallbackQuery(callbackQueryId, { text: "❌ Ошибка данных" });

                // Вызываем Сервис (Логика там)
                const financeData = await OrderService.updateStatus(orderId, newStatus, userId);

                // Обновляем UI
                const time = new Date().toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' });
                const cleanBody = (message.text || '').replace(/^.*(СТАТУС|Мастер|Обновлено):.*\n/gm, '').trim();
                ...
                return bot.answerCallbackQuery(callbackQueryId, { text: `✅ Статус: ${cfg.label}` });
            }
            ...
            if (data.startsWith('take_order_')) {
                ...
                try {
                    ...
                    return bot.answerCallbackQuery(callbackQueryId, { text: "🚀 Успешно!" });
                } catch (e) {
                    if (e.message === 'ACCESS_DENIED') {
                        return bot.answerCallbackQuery(callbackQueryId, { text: "⛔️ Нет прав", show_alert: true });
                    }
                    throw e;
                }
            }
            ...
            if (data.startsWith('wall_')) {
                ...
                if (!session?.data?.area) {
                    return bot.answerCallbackQuery(callbackQueryId, { text: "⚠️ Сессия истекла", show_alert: true });
                }
                ...
                const estimate = await OrderService.calculateEstimate(session.data.area, wallType);

                // 2. Сохраняем Лид (через Сервис)
                const leadId = await OrderService.createLead(userId, estimate);

                // 3. Рисуем ответ
                const wallNames = { light: 'ГКЛ/Газоблок', medium: 'Кирпич', heavy: 'Бетон' };
                const txt = 
                    `⚡️ <b>СМЕТА</b>\n` +
                ...
                await bot.sendMessage(chatId, txt, {
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: "📝 Оформить заявку", callback_data: `order_chat_${leadId}` }]
                        ]
                    }
                });
            ...
            if (data.startsWith('order_')) {
                ...
                await bot.sendMessage(chatId, "✅ <b>Заявка принята!</b>\nМенеджер свяжется с вами.", { parse_mode: 'HTML' });
            ...
            await bot.answerCallbackQuery(callbackQueryId, { text: "Ошибка сервера" });
        }
    });
};